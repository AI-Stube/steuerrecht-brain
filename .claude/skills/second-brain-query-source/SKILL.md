...
### 4. Check archive sources if needed

If the wiki pages don't fully answer the question, check relevant source summaries in `wiki/sources/` for additional detail. If still insufficient, read the full original document from `archive/` — the path is in the `sources:` frontmatter of each wiki summary page.
...
- **Search the wiki first.** Only go to `archive/` originals if the wiki doesn't have enough detail.
